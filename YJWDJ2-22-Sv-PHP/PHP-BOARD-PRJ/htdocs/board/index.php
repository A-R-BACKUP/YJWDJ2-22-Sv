...
// 검색된 게시물 목록
$stmt2 = $db->prepare("SELECT * FROM " . $_board_options["tableName"] . $sql_where . " ORDER BY idx DESC LIMIT " . $sql_start_index . ", " . $page_max_rows . ";");
if($q != '') {
...
<html>
    <head>
        ...
        <style>
            ...
            .td-attach-file { width: 60px; text-align: center; }
            .td-view-count { width: 80px; text-align: center; }
    ...
    <body>
        <div id="main" class="container-fluid h-100 px-0">
            ...
            <div class="w-100 px-3">
                ...
                <div>
                    ...
                    <?php for($i=$start_page_block; $i<=$end_page_block; $i++) {?>
                        <a href="?page_no=<?=$i?>&q=<?=$q?>" class="btn btn-sm btn<?php if($page_no!=$i){echo '-outline';}?>-secondary"><?=$i?></a>
                    <?php }?>
            ...
            function search() {
                let options = [];
                options.push({type: 'text', selector : '#query', regex: /^.{0,1}$/, isFocus: true, errorMsg: '검색어를 2글자 이상 입력하세요.'});
